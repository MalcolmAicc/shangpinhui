          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <!-- <dd changepirce="40">{{skuSaleAttr}}</dd>
...
export default {
  ...
  methods: {
    changeNum() {
      // 将用户输入的内容*1，如果是字符串类型，结果会是NaN，就可以将输入内容筛选出来
    ...
    async addCart(){
      // 参数传递是一个对象
      // console.log(this.skuInfo.id,this.skuNum);
      try {
        await this.$store.dispatch('detail/getCartInfo',{skuId:this.skuInfo.id,skuNum:this.skuNum})
        alert('加入购物车成功')
        // skuInfo通过会话存储的形式存储
        // 会话存储只能存储字符串格式，存储对象需要用JSON转码
