  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        收货人：<span>{{ addressInfo.consignee }}</span>
...
import {mapState} from 'vuex'
  export default {
    ...
    computed:{
      ...
      // 计算出最底下的地址和收件人信息
      addressInfo(){
        // find方法用来筛选数组中满足条件的元素,并返回
        return this.address.find(item => item.isDefault === 1) || {}
      },
    },
    methods: {
      changeSelect(address){
        // 排他思想，先将数组里面的每个对象的isDefault更改为0，再将点击的那个改为1
        this.address.find(element => {
          element.isDefault = 0
        });
        address.isDefault = 1
      },
      // 提交订单
      async submitOrder(){
        let {tradeNo} = this.OrderInfo // 产品编号
        let data = {
          consignee: this.addressInfo.consignee, // 收件人姓名
          consigneeTel: this.addressInfo.phoneNum, // 收件人手机号哈
          deliveryAddress: this.addressInfo.fullAddress, // 收件地址
          paymentWay: "ONLINE",  // 支付方式
          orderComment: this.message,  // 买家留言
          orderDetailList: this.OrderInfo.detailArrayList  // 商品信息
        }
        // 需要携带参数，tradeNo，和data
