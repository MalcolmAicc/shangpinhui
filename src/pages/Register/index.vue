...
export default {
  ...
  methods: {
    async getCode() {
      // 一点击就可以将验证码传到文本框中必须要用try/catch语法
      try {
        if (this.phone) {
          await this.$store.dispatch("users/getCode", this.phone);
          // 将code写为返回的code
    ...
    // 用户注册
    async userRegister() {
      // 简单判断是否有手机号，验证码，password是否等于password1
      try {
        ...
        if ((phone, code, password === password1)) {
          await this.$store.dispatch("users/userRegister", { phone, code, password });
          this.$router.push('/login')
        }
      } catch (error) {
          alert('注册失败，请确保内容填写完整！')
      }
    },
  },
};
