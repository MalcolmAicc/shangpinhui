    ...
    <div class="floor">
      <div class="py-container">
        <div class="title clearfix">
          <h3 class="fl">{{ listProps.name }}</h3>
          <div class="fr">
            <ul class="nav-tabs clearfix">
              <li
                class="active"
                v-for="(a, index) in listProps.navList"
                :key="index"
              >
        ...
        <div class="tab-content">
          <div class="tab-pane">
            <div class="floor-1">
              <div class="blockgary">
                <ul class="jd-list">
                  <li v-for="(key, index) in listProps.keywords" :key="index">
                    {{ key }}
                  </li>
                </ul>
                <img :src="listProps.imgUrl" />
              </div>
              <div class="floorBanner">
                <Carousel :list="listProps.carouselList" />
              </div>
              <div class="split">
                <span class="floor-x-line"></span>
                <div class="floor-conver-pit">
                  <img :src="listProps.recommendList[0]" />
                </div>
                <div class="floor-conver-pit">
                  <img :src="listProps.recommendList[1]" />
                </div>
              </div>
              <div class="split center">
                <img :src="listProps.bigImg" />
              </div>
              <div class="split">
                <span class="floor-x-line"></span>
                <div class="floor-conver-pit">
                  <img :src="listProps.recommendList[2]" />
                </div>
                <div class="floor-conver-pit">
                  <img :src="listProps.recommendList[3]" />
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  name: "Floor",
  props: ["listProps"],
  // 注意 swiper 在这里的数据是父组件传过来的  不存在异步的操作所以组件挂载之前结构已经存在
};
</script>
